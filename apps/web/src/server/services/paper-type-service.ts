...
export async function createPaperTypeOption(name: string, label?: string | null) {
  try {
    ...
    if (!trimmed) return { error: "Paper type name is required." };

    const existing = await db.paperTypeOption.findFirst({ where: { name: trimmed } });
    if (existing) return { error: `Paper type "${trimmed}" already exists.` };
    ...
    return { paperType: created };
  } catch (err: any) {
    return { error: err?.message || "Failed to create paper type." };
  }
}
...
export async function updatePaperTypeOption(id: string, name: string, label?: string | null) {
  try {
    ...
    if (!existing) return { error: "Paper type not found." };

    if (existing.name !== trimmed) {
      const dup = await db.paperTypeOption.findFirst({ where: { name: trimmed } });
      if (dup && dup.id !== id) return { error: `Paper type "${trimmed}" already exists.` };
    }
    ...
    return { paperType: updated };
  } catch (err: any) {
    return { error: err?.message || "Failed to update paper type." };
  }
}
...
export async function deletePaperTypeOption(id: string) {
  try {
    ...
    if (ordersInUse > 0 || stockInUse > 0) {
      return {
        error: `"${existing.name}" is used by ${ordersInUse} order line(s) and ${stockInUse} stock item(s). Reassign or delete those first.`,
      };
    }
    ...
    return { error: err?.message || "Failed to delete paper type." };
  }
}
